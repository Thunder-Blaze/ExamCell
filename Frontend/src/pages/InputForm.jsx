...
const InputForm = () => {
  ...
  const courseOptions = useMemo(() => programCourses[formData.program] || [], [formData.program])
  const maxSemester =
  ...
  useEffect(() => {
    let selected = courseOptions.filter((course) => formData.rollNumber.includes(course));
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-12">
        <Card className="w-full max-w-xl shadow-xl border rounded-2xl bg-card text-card-foreground">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl font-semibold mb-1">📄{hasDetails ? " Update" : " Submit"} Student Details</CardTitle>
            <p className="text-sm text-muted-foreground">Please fill out all fields carefully</p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button type="submit" className="w-full text-lg font-semibold py-3" disabled={isLoading}>
                {isLoading ? 
                <span className="animate-pulse">Submitting...</span>
                : (hasDetails ? "Update Details" : "Submit Details")}
              </Button>
            </form>
          </CardContent>
        </Card>
